...
| 6 — deploy | **done** 2026-09-24 | `~/bin/KnurLogger.ini` backed up to `~/bin/KnurLogger.ini.bak-20260924`, then hand-merged: 27 comment lines and the 32 new keys inserted, nothing else changed, and its values match the template exactly. The owner deployed with `deploy-logger.sh --execute` and started the service at 19:05:04; it is active with 0 restarts, and `~/bin/KnurLogger` is byte-identical to the build. Session `2026-09-24T17-05-04.760174Z`: the header's `correction` matches the production `.ini` key for key, and `pressure-correction-check.py` passes (largest difference 0.00007 Pa, no stale periods). Production's comments predated the template's cleanup, so at the owner's request they were brought to the template's wording (values checked identical first; the merged file kept as `~/bin/KnurLogger.ini.bak-20260924-merged`): production is now byte-identical to the committed `build/KnurLogger.ini`, so the copy back changes nothing. Phone check passed (owner, 2026-09-24) |
