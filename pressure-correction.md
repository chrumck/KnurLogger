...
| 6 — deploy | **done** 2026-09-24, phone check and copy-back open | `~/bin/KnurLogger.ini` backed up to `~/bin/KnurLogger.ini.bak-20260924`, then hand-merged: 27 comment lines and the 32 new keys inserted, nothing else changed, and its values match the template exactly. The owner deployed with `deploy-logger.sh --execute` and started the service at 19:05:04; it is active with 0 restarts, and `~/bin/KnurLogger` is byte-identical to the build. Session `2026-09-24T17-05-04.760174Z`: the header's `correction` matches the production `.ini` key for key, and `pressure-correction-check.py` passes (largest difference 0.00007 Pa, no stale periods). **Open:** the phone check, and the copy back into `build/KnurLogger.ini` — production's values already equal the template, but its comments predate the template's cleanup, so copying it would undo that; the owner decides |
