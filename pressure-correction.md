...
| 3 — the correction | **done** 2026-09-24 | `getLinePathResistance()`, `correctPressure()`, `applyPressureCorrection()` and a one-shot stale/recover event pair in `pressureSensors.cxx`; `SdpReading` gains `correctedPa`/`isCorrected`, and `pressureDeciPa` is now the corrected value or `INT16_MIN`. A valid-but-uncorrected channel keeps `valid: true` with reason `absolutePressureMissing`, `absolutePressureStale` or `correctedOutOfRange` (past the wire's int16). A throwaway harness, never committed, built against the real sources on the box: every reference row within 0.0004 Pa, the template's `P1` loaded correctly, the sentinel cases send `INT16_MIN` with the raw value unchanged, and the hold holds at 9.9 s and goes stale at 10.1 s. The worker waits up to 2 s at start for the first absolute pressure: three hand-runs sent corrected values from the first cycle and logged no absolute-pressure event, and a harness showed the wait ending 3 ms after the value appears and giving up at 2003 ms without one. |
| 4 — records and packets | **done** 2026-09-24 | Each channel gains `correctedPa`; the record gains `absolutePressurePa`/`absolutePressureAgeMs` (the held value the cycle read, kept when stale so its age explains the refusal; null when none yet) and `sentMask`, which is what `0x607` byte 1 now carries, while `validMask` stays the raw validity. `pressureBaseline` gains `correction` (formula, calibration pressure, hold, `r`, `R_fixed`, per enabled slot its six values and `R_path`) and its `wireScaling` text says the air carries the corrected value. Hand-run on the box: the fields are present, and a temporary packet print, never committed, showed `0x605`/`0x606` equal to the records' `sentDeciPa` on 3 of 3 sampled cycles. In still air the corrections are hundredths of a pascal, so the live run shows the plumbing; the harness of step 3 shows the arithmetic. The stale path's `sentMask` was not exercised live |
| 5 — offline check | **done** 2026-09-24 | `Tools/pressure-correction-check.py`: recomputes each `correctedPa` from `pressurePa`, `absolutePressurePa` and the baseline's `correction`, checks `sentDeciPa`, explains every uncorrected valid reading, and matches stale-hold periods to their events; exit 1 on any failure. On the step 4 session (`2026-09-24T16-49-21.965026Z`) the largest difference is 0.00005 Pa, with 0 stale periods and 0 events. On a 20 s hand-run from a scratch `.ini` with lines on `P0` (10.088 m), `P2` (1.5 m) and `P3` (3 + 3 m) it is 0.00017 Pa, with `R_path` as computed by hand. A doctored copy with a stale period and its two events passes, and one with a single value off by 0.05 Pa fails. No real session has had a stale period yet |
| 6 — deploy | **done** 2026-09-24, phone check open | `~/bin/KnurLogger.ini` backed up to `~/bin/KnurLogger.ini.bak-20260924`, then hand-merged: 27 comment lines and the 32 new keys inserted, nothing else changed, and its values match the template exactly. The owner deployed with `deploy-logger.sh --execute` and started the service at 19:05:04; it is active with 0 restarts, and `~/bin/KnurLogger` is byte-identical to the build. Session `2026-09-24T17-05-04.760174Z`: the header's `correction` matches the production `.ini` key for key, and `pressure-correction-check.py` passes (largest difference 0.00007 Pa, no stale periods). Production's comments predated the template's cleanup, so at the owner's request they were brought to the template's wording (values checked identical first; the merged file kept as `~/bin/KnurLogger.ini.bak-20260924-merged`): production is now byte-identical to the committed `build/KnurLogger.ini`, so the copy back changes nothing. **Open:** the phone check |
| 7 — documentation | **done** 2026-09-24 | `racechrono-channels.md` (the air carries the corrected value; `-32768` also for an uncorrectable reading; `0x607` byte 1 is the sent mask), `session-records.md` (new §Pressure correction), `operations.md` (new §Pressure correction settings; the forward-looking crash-loop note retired), README task map and Next; ndLouvers `pressure-testing.md` §2.4 step 4 and its status note, `instrumentation-spec.md` Step 0b note, `work-progress.md` reading path and Step 0b row. History entry added; link checker 0 findings |
